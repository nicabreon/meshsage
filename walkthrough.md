# Walkthrough: Developer Guide, Architectural Design, & E2E Verification (Meshsage Runtime)

This document is a comprehensive guide to the development, architectural components, protocol workflows, and verification evidence of the **Meshsage Runtime**—a distributed, decentralized peer-to-peer (P2P) messaging library and runtime.

---

## 1. Architectural Component Layering

The Meshsage Runtime is structured to handle all cryptographic operations, network routing, and local SQLite data persistence:
...
*   **Scenario 4: Group Chat (Alternating Offline):** Simulates alternating offline sequences and verifies offline group mailbox storage and recovery.
...
## 5. Runtime Stability Bugfixes

### A. E2E Test Suite Execution Lock Resolution
...
3. **ZKP Envelope Wrapping**: When storing offline messages in the mailbox, the envelope is wrapped in a ZKP ring signature: proving membership in the active registered user ring without revealing *which* member signed it.
...
## 7. Walkthrough: TUI Concurrent Buffer Reuse Fix
*   **Problem:** On the Terminal User Interface (TUI), chat messages and system logs would occasionally get garbled or scrambled due to concurrent buffer reuse of the byte slice.
*   **Changes:** Updated type definitions of `logMsg` and `chatMsg` in `pkg/tui/tui.go` from `[]byte` to `string` and converted the slice immediately, forcing a copy of the data.

---

## 8. Walkthrough: ZKP Startup Clean Fix
*   **Problem:** The mailbox/relay node would sometimes reject the envelope with a ZKP verification error because the sender's local key database contained old, stale, or residual third-party ZKP keys not present on the relay.
*   **Changes:** Added a startup database cleanup function `CleanZKPMembersExceptOwner` to clear third-party keys. Real-time active nodes automatically populate verified keys dynamically during active sessions.

---

## 9. Walkthrough: Proactive Session Warm-up Probe (Runtime side)
*   **Problem:** Establishing a session or discovering decryption issues only happened during active user messaging, causing latency on the first message.
*   **Changes:** Implemented `HasSession` to verify DB state, and `ProbeSessionWarmup` to trigger silent `MsgTypeHandshakeAck` envelopes to establish connection validity on startup.

---

## 10. Walkthrough: Fixed Group Invite Signature Verification (Timestamp Drift Fix)
*   **Problem:** Timestamp drift between group creation metadata signing and the database insertion time caused group invite signature verification to fail on recipients.
*   **Changes:** Passed explicit `createdAt` unix timestamps to `JoinGroupProper` and propagated the signed creator timestamp properly.

---

## 11. Walkthrough: Offline Invitation Envelope Sorting & Group Alias PM Block
*   **Problem:** Message ordering issues on mailbox fetches caused `DR` envelopes to be processed before `X3DH` envelopes, causing session erasures. Private messages could also be sent to group aliases incorrectly.
*   **Changes:** Implemented envelope sorting to process handshakes first, and added a group check validation to `resolveTargetPeerID` to prevent private message delivery to group aliases.

---

## 12. Walkthrough: Media File Replication to Dedicated Relays
*   **Problem:** Senders behind symmetric NATs could not have their files downloaded via direct Bitswap by third-party peers.
*   **Changes:** Added a `REPLICATE` command to `/p2p-core/mailbox/1.0.0` protocol allowing relay nodes to download and cache files to replicate them.

---

## 13. Walkthrough: Disabled Session Warm-up & Enhanced Mailbox Diagnostic Logs
*   **Problem:** Session warmup probes went to mailboxes, causing empty fetches.
*   **Changes:** Disabled warmup mailbox fallbacks and enhanced explicit warning/error logging inside `FetchMailboxMessages`.

---

## 14. Walkthrough: Relay Rate Limit Reduction to 1ms
*   **Problem:** Startup concurrent prekey refills and fetch queries triggered relay rate limiting.
*   **Changes:** Reduced the rate-limit window from 50ms to 1ms on relays.

---

## 15. Walkthrough: Mailbox Deduplication Retrieval & Group Message ID Fixes
*   **Problem:** DB query scan errors ignored duplicate recovery. Group message IDs were empty, discarding group messages on the UI side.
*   **Changes:** Wrapped scan queries in `COALESCE` to handle NULLs, generated unique group message IDs from signatures (`gr-<hash>`), and cached offline group messages in SQLite.

---

## 16. Walkthrough: Sequential Global Mailbox Sync Manager
*   **Problem:** Spawning a sync loop per relay caused high mobile overhead and rate-limit conflicts.
*   **Changes:** Refactored mailbox sync to run sequentially across active relays in a single background manager.

---

## 17. Walkthrough: Reliable Offline Group Messages & Direct Key Requests
*   **Problem:** Offline members could not decrypt offline group messages because they lacked the group key history.
*   **Changes:** Implemented direct key requests (`GREQ`) where members request historical group keys securely from the creator over 1:1 E2EE.

---

## 18. Walkthrough: Group Message Reliability & Stale Connection Verification
*   **Problem:** Messages were lost when writing to stale connections.
*   **Changes:** Added connection-check confirmation and auto-reconnect logic inside the Go-core stream writing routines.

---

## 19. Walkthrough: Group Creator Registry Fix for Offline Mailbox Delivery
*   **Problem:** Group creator was omitted from invitation lists, resulting in no mailbox fanning out to the creator.
*   **Changes:** Explicitly registered the creator in the group member list on the recipient's local database.

---

## 20. Walkthrough: Proactive Connection Upgrade on Chatroom Open (Runtime side)
*   **Changes:** Implemented `ConnectPeer` FFI entrypoint that performs DHT peer routing and attempts a direct connection.

---

## 21. Walkthrough: Android Foreground Lifecycle Sync & Stream Dial Timeout Optimization (Runtime side)
*   **Changes:** Reduced context stream dial timeout from 3 seconds to 100ms to allow extremely fast fallback to mailbox storage.

---

## 22. Walkthrough: Fixed Manual Mailbox Fetch Relay Filtering (Client-Only Exclusion)
*   **Changes:** Filtered active peers by `InfrastructureProtocolID` (`/p2p-core/infra/1.1.0`) during fetches to exclude client-only peers.

---

## 23. Walkthrough: Explicit Message Delivery Route Logging
*   **Changes:** Logged explicit route indicators (`DIRECT (QUIC/UDP)`, `DIRECT (WebRTC)`, `RELAYED (Circuit)`) to the console logger.
